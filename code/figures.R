setwd('/Users/eleanorbrush/Dropbox/evo_badgesVSrecognition/code')
...
param_plot <- function(data,mapping,ymin,ymax,ybreaks=NULL,ylabels=NULL,indiv_agg = FALSE,to_agg = NULL,interaction=FALSE){	
	if(indiv_agg){
	...
	if(interaction){	
		plot = plot+scale_color_manual(values=c(divpal[1],divpal[1],divpal[5]),labels=c(paste('Badge',corr_vals[1],sep=', '),paste('Badge',corr_vals[2],sep=', '),'Indiv'))+scale_linetype_manual(values=c(5,1,1),labels=c(paste('Badge',corr_vals[1],sep=', '),paste('Badge',corr_vals[2],sep=', '),'Indiv'))}
	else{
		plot = plot +scale_color_manual(values=c(divpal[1],divpal[5]))+scale_linetype_manual(values=c(3,1,5))} 
	plot = plot +
	theme_bw() +
...
for(q in 1:((dim(parameters_toplot)[1])/xcorr)){
    ...
    geom_line(data=error_now,aes(x=fights,y=error,linetype=interaction(sigcorr,categ),color=interaction(sigcorr,categ)))+
    guides(fill=FALSE,colour=guide_legend(override.aes=list(fill=NA)))+ 
	scale_linetype_manual(values=c(5,1,1),labels=c(paste('Badge',corr_vals[1],sep=', '),paste('Badge',corr_vals[2],sep=', '),'Indiv')) +	scale_color_manual(values=divpal[c(1,1,5)],labels=c(paste('Badge',corr_vals[1],sep=', '),paste('Badge',corr_vals[2],sep=', '),'Indiv'))+
	...
	theme(legend.position='none',legend.justification=c(1,1),legend.box.just = "left"
...
plots.error[[1]] = param_plot(error_N, aes(x=N, y = error, colour = interaction(categ,c2), linetype = interaction(categ,c2)),0,error_max,interaction=TRUE) + 
	xlab("Group size") + ylab("Error") 	+ theme(legend.position=c(0.6,0.28),legend.box.just = "left"
...
plots.error[[4]] = param_plot(error_p, aes(x=p, y = error, colour = interaction(categ,c2), linetype = interaction(categ,c2)),0.2,0.44,interaction=TRUE) + 	
	xlab("Probability of recategorizing")+ylab("Error") + theme(legend.position=c(0.2,0.87),legend.box.just = "left"
,legend.box='horizontal',plot.margin = unit(x = marg, units = "mm"),legend.key.size=unit(.2,'inch'))+transparent_legend
...
plots.error[[5]] = param_plot(error_pobs, aes(x=pobs, y = error, colour = interaction(categ,c2), linetype = interaction(categ,c2)),0,0.45,interaction=TRUE) + 	
	xlab("Prob of observing")+ ylab("Error") + theme(legend.position='none')
	# theme(legend.position=c(0.2,0.96),legend.direction='vertical',legend.box.just = "left",legend.box='horizontal',legend.key.size=unit(.13,'inch'),legend.key.width=unit(.2,'inch'),plot.margin = unit(x = marg, units = "inch"))+transparent_legend
...
plots.time[[5]] = param_plot(time_pobs, aes(x=pobs, y = time, colour = interaction(categ,c2), linetype = interaction(categ,c2)),log(430,base=10),log(8000,base=10),ybreaks=time_breaks,ylabels=time_labels,interaction=TRUE) + 	
	xlab("Prob of observing")+ ylab("Time") + theme(legend.position=c(0.28,0.22),legend.direction='vertical',legend.box.just = "left"
,legend.box='horizontal',legend.key.size=unit(.13,'inch'),legend.key.width=unit(.2,'inch'),plot.margin = unit(x = marg, units = "inch"))+transparent_legend
	# +theme(legend.position='none')#+ theme(legend.position=c(0.53,0.94),legend.box.just = "left"
...
legend_breaks = round(seq(m,M-.01,length.out=5),1)
...
plots.cost.pw[[c2]] = ggplot(cost_pw_subset,aes(x=pobs,y=w,z=diff))+geom_tile(aes(fill = diff)) +  
	...
	theme(text=element_text(family="Helvetica", size=textsz), plot.title=element_text(size=textsz), plot.margin=unit(marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
	scale_y_continuous(expand = c(0,0)) + scale_x_continuous(expand = c(0,0)) +
	ylab('Memory window') + xlab('Prob of observing')
}
...
widths=c(1,1,0.4)
layout = array(NA,dim=c(4,3))
layout[1:9]=1:9
grid.arrange(plots.cost.Nw[[1]],plots.cost.Nc[[1]],plots.cost.cw[[1]],plots.cost.pw[[1]],plots.cost.Nw[[2]],plots.cost.Nc[[2]],plots.cost.cw[[2]],plots.cost.pw[[2]],contour_leg_cost,ncol=3,widths=widths,layout_matrix=layout)
dev.off()
