...
parameters_cat_toplot = parameters_cat
parameters_cat_toplot$w[parameters_cat$w==Inf] = 3000

parameters_rule_toplot = parameters_rule
parameters_rule_toplot$w[parameters_rule$w==Inf] = 3000
...
optimization<- function(w){	
	...
	for(i in 1:dim(parameters_rule)[1]){
		...
		if(max(d_var[start3:length(d_var)],na.rm=TRUE)<4e-05){				
			offset = 6
			s = sign(myround(d_avg,3.1)) ##change from 3.15
		}else{
			offset = 10
			s = sign(myround(d_avg,3))	
		}
...
optimization_rule<- function(error,w){		
	...
	offset = 5
	s = sign(myround(d_avg,3))	
	if(max(d_var[start3:length(d_var)],na.rm=TRUE)<1.5e-08){	##changed from 4e-05			 
		offset = 6 ##changed from 4
		s = sign(myround(d_avg,3.1)) ##changed from 3.15
	}else{
		offset = 10
		s = sign(myround(d_avg,3))	
	}
...
#### learning curves
examples=data.frame(N=rep(NA,4),rho=rep(0.99,4),w=c(2500 ,rep(2500,2),rep(1000)),delta=c(0,0,0.25,0.25),l=c(0.05,0.5,0.05,0.05))
...
f_cat = find_df(examples)
f_rule = find_df(examples[,rule_relevant],p=parameters_rule)
f_rule[c(1,2),]=NA
...
for(i in 1:num_ex){
	legend_labels = c(legend_labels,bquote(.(paste(' w=',examples$w[i],', r=',examples$l[i],', ',sep=''))*delta*.(paste('=',examples$delta[i],sep=''))))
	}
legend_labels=c(legend_labels,'Categorization','Learned rule','Innate rule')
legend_labels = sapply(legend_labels,as.expression)

width = 6.85
height =2.5

marg = c(0.43,0.42,0.04,0.1)
omarg = c(0.05,0.33,0.3,0.0)

pdf('/Users/eleanorbrush/Desktop/learning_curves.pdf',width=width,height=height,family=fontfamily)
...
xlim=c(-0.01,30000)
for(j in 1:3){
plot(time,time,t='n',ylim=ylim,xlim=xlim,xlab='',ylab='',yaxs='i',xaxs='i')
for(i in 1:num_ex){
	lines(time,error_cat[,f_cat[i,j]],col=mycols[i])
	lines(time,error_rule[,f_rule[i,j]],col=mycols[i],lty=2)
	lines(time,error_genetic[,f_gen[i,j]],col='black',lty=3)
	if(j==1){
		legend(2650,0.43,legend=legend_labels,lty=c(rep(1,num_ex),1,2,3),col=c(mycols[1:num_ex],'black','black','black'),bty='n')
	}
}
mtext(expression(paste('Error, ',epsilon,sep='')),2,line=2,at=mean(ylim),cex=largefontsize/smallfontsize)
mtext('(t)',2,line=2.2,at=mean(ylim)+0.073,cex=1)
mtext('Time',1,line=2.3,at=mean(time),cex=largefontsize/smallfontsize)
}
...
best_cat = opt$best_cat
optimized_cat = opt$optimized_cat/2
best_rule = opt$best_rule
optimized_rule = opt$optimized_rule/2
best_gen = rep(squish(10),dim(parameters_genetic)[1])
optimized_gen = error_genetic[1,]

marg = c(0.45,0.4,0.04,0.05)
...
examples = data.frame(N=c(25,25),rho=c(0.9,0.99),w=c(250,250),delta=c(0,0),l=c(0.25,0.25))
num_ex = dim(examples)[1]
rule_relevant = intersect(which(names(examples)!='delta'),which(names(examples)!='l'))
gen_relevant = rule_relevant[which(names(examples)[rule_relevant]!='w')]
...
for(i in 1:num_ex){
	...
	rest_of_label = paste('=',examples$rho[i],', w=',examples$w[i],sep='')
	legend_labels = c(legend_labels,bquote(rho*.(rest_of_label)))
	}
legend_labels=c(legend_labels,'Categorization','Learned rule','Innate rule')
legend_labels = sapply(legend_labels,as.expression)

parameters_toplot=c('N','w','l','delta')
labels = c('Group size, N','Memory window, w','Updating rate, r',expression(paste('Category width, ',delta,sep='')))

time_ylim = squish(c(400,40001))
...
for(var in parameters_toplot){
	...
	plot(parameters_cat_toplot[[var]],optimized_cat,t='n',xlab='',ylab='',xaxt='n',yaxt='n')
	axis(1,mgp=c(3,0.5,0))
	axis(1,at=3000,labels=30000,mgp=c(3,0.5,0))	
	axis(2,mgp=c(3,0.5,0))
	for(i in 1:num_ex){
		points(parameters_cat_toplot[[var]][f_cat[i,]],optimized_cat[f_cat[i,]],t='o',col=mycols[i])
	}	
	for(i in 1:dim(f_rule)[1]){
		if(is.element(var,names(parameters_rule)[rule_relevant])){
			points(parameters_rule_toplot[[var]][f_rule[i,]],optimized_rule[f_rule[i,]],t='o',col=mycols[i],lty=2)
		}else{
			points(unique(parameters_cat_toplot[[var]]),rep(optimized_rule[f_rule[i,1]],length(unique(parameters_cat[[var]]))),t='l',col=mycols[i],lty=2)
		}		
	}
	for(i in 1:dim(f_gen)[1]){
		if(is.element(var,names(parameters_genetic)[gen_relevant])){
			points(parameters_genetic[[var]][f_gen[i,]],optimized_gen[f_gen[i,]],t='o',col=mycols[i],lty=3)
		}else{
			points(unique(parameters_cat_toplot[[var]]),rep(optimized_gen[f_gen[i,1]],length(unique(parameters_cat[[var]]))),t='l',col=mycols[i],lty=3)
		}		
	}	
	xlim = range(unique(parameters_cat_toplot[[var]]))
	mtext(labels[which(parameters_toplot==var)],1,line=2.5,at=0.55*xlim[1]+0.45*xlim[2],cex=largefontsize/smallfontsize)
	ylim=range(optimized_cat,na.rm=TRUE)
	mtext(expression(paste('Error, ',epsilon(tau),sep='')),2,line=1.4,at=mean(ylim),cex=largefontsize/smallfontsize)
	
	plot(parameters_cat_toplot[[var]],squish(time[best_cat]),t='n',xlab='',ylab='',ylim=time_ylim,yaxt='n',xaxt='n')
	axis(1,mgp=c(3,0.5,0))
	axis(1,mgp=c(3,0.5,0),at=3000,labels=30000)
	ticks = c(1,1000,4000,9000,16000,25000)
	ticks = c(1000,4000,16000)
	axis(2,at=squish(ticks),labels=ticks,mgp=c(3,0.5,0))
	for(i in 1:num_ex){
		points(parameters_cat_toplot[[var]][f_cat[i,]],squish(time[best_cat[f_cat[i,]]]),t='o',col=mycols[i])
	}	
	for(i in 1:dim(f_rule)[1]){
		if(is.element(var,names(parameters_rule)[rule_relevant])){
			points(parameters_rule_toplot[[var]][f_rule[i,]],squish(time[best_rule[f_rule[i,]]]),t='o',col=mycols[i],lty=2)
		}else{
			points(unique(parameters_cat_toplot[[var]]),rep(squish(time[best_rule[f_rule[i,1]]]),length(unique(parameters_cat[[var]]))),t='l',col=mycols[i],lty=2)
		}		
	}
	for(i in 1:dim(f_gen)[1]){
		if(is.element(var,names(parameters_genetic)[gen_relevant])){
			points(parameters_genetic[[var]][f_gen[i,]],best_gen[f_gen[i,]],t='o',col=mycols[i],lty=3)
		}else{
			points(unique(parameters_cat_toplot[[var]]),rep(best_gen[f_gen[i,1]],length(unique(parameters_cat[[var]]))),t='l',col=mycols[i],lty=3)
		}		
	}
	xlim = range(unique(parameters_cat_toplot[[var]]))
	mtext(labels[which(parameters_toplot==var)],1,line=2.5,at=0.55*xlim[1]+0.45*xlim[2],cex=largefontsize/smallfontsize)
	mtext(expression(paste('Stopping time, ',tau,sep='')),2,line=1.4,at=mean(time_ylim),cex=largefontsize/smallfontsize)
}	


legend(-0.10,y=11,legend=legend_labels,lty=c(rep(1,num_ex),1,2,3),col=c(mycols[1:num_ex],'black','black','black'),bty='n',seg.len=1.2)
...
rainbow = brewer.pal(8,'Spectral')[-5]
...
f = rev(find(N=N,rho=rho,delta=delta,l=l)[-1])
plot(time,time,ylim=ylim,xlim=xlim,t='n',xlab='',ylab='',xaxt='n',yaxt='n',xaxs='i',yaxs='i')
axis(1,mgp=c(3,0.5,0))
axis(2,mgp=c(3,0.5,0))
for(i in 1:length(f)){
	lines(time,error_cat[,f[i]],t='l',col=rainbow[i],lwd=lwd)
}
mtext(expression(paste('Error, ',epsilon,sep='')),2,line=1.6,at=mean(ylim),cex=largefontsize/smallfontsize)
mtext('(t)',2,line=1.8,at=mean(ylim)+0.073,cex=1)
mtext('Time',1,line=2.3,at=mean(time),cex=largefontsize/smallfontsize)
w_leg = unique(parameters_cat$w[f])
w_leg[w_leg==Inf] = 30000
legend(15000,0.51,legend=w_leg,col=rainbow[1:length(f)],lty=1,lwd=2,bty='n')
...
mtext('(t)',2,line=1.8,at=mean(ylim)+0.073,cex=1)
mtext('Time',1,line=2.3,at=mean(time),cex=largefontsize/smallfontsize)
legend(15000,0.51,legend=unique(parameters_cat$l[f]),col=rainbow[1:length(f)],lty=1,lwd=2,bty='n')

f = find(N=N,rho=rho,l=l,w=w)
plot(time,time,ylim=ylim,xlim=xlim,t='n',xlab='',ylab='',xaxt='n',yaxt='n',xaxs='i',yaxs='i')
axis(1,mgp=c(3,0.5,0))
axis(2,mgp=c(3,0.5,0))
for(i in 1:length(f)){
...
mtext('(t)',2,line=1.8,at=mean(ylim)+0.073,cex=1)
mtext('Time',1,line=2.3,at=mean(time),cex=largefontsize/smallfontsize)
legend(15000,0.51,legend=unique(parameters_cat$delta[f]),col=rainbow[1:length(f)],lty=1,lwd=2,bty='n')

dev.off()

######## parameter interaction heat maps
seqpal = rev(brewer.pal(9,'Reds'))
seqpal2 = rev(brewer.pal(9,'Blues')[2:9])
# seqpal2 = c(rep('black',2),seqpal2)
...
error_delta_w = data.frame(delta=parameters_cat$delta[f],w=parameters_cat$w[f],error=optimized_cat[f]/2)
...
error_delta_l = data.frame(delta=parameters_cat$delta[f],l=parameters_cat$l[f],error=optimized_cat[f]/2)
...
error_w_l = data.frame(w=parameters_cat$w[f],l=parameters_cat$l[f],error=optimized_cat[f]/2)
...
contour_error[[1]] = ggplot(error_delta_w,aes(y=w,x=delta,z=error)) +
	...
	scale_fill_gradientn(colours=rev(seqpal),limits=M_error,guide='colorbar')+
	...
	scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) + scale_x_continuous(expand = c(0,0),breaks=seq(0,1,by=0.25))+
...
contour_error[[3]] = ggplot(error_w_l,aes(y=w,x=l,z=error)) +
	...
	scale_fill_gradientn(colours=rev(seqpal),limits=M_error,guide='colorbar')+
	theme_bw()+ theme(text=element_text(family="Helvetica", size=smallfontsize), plot.margin=unit(big_marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
	scale_x_continuous(expand = c(0,0),breaks=unique(error_delta_l$l),labels=unique(parameters_cat$l)) + scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) +
...
contour_time[[1]] = ggplot(time_delta_w,aes(y=w,x=delta,z=time)) +
	...
	scale_fill_gradientn(colours=rev(seqpal2),limits=M_time,guide='colorbar')+
	theme_bw()+ theme(text=element_text(family="Helvetica", size=smallfontsize), plot.margin=unit(big_marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
	scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) + scale_x_continuous(expand = c(0,0),breaks=seq(0,1,by=0.25))+
	ylab('Memory window, w') + xlab(expression(paste('Category width, ',delta,sep='')))
...
contour_time[[2]] = ggplot(time_delta_l,aes(y=l,x=delta,z=time)) +
	...
	scale_fill_gradientn(colours=rev(seqpal2),limits=M_time,guide='colorbar',na.value=brewer.pal(9,'YlGnBu')[9])+
	theme_bw()+ theme(text=element_text(family="Helvetica", size=smallfontsize), plot.margin=unit(big_marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
...
error_legend = ggplot(error_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=z)) + scale_fill_gradientn(colours=rev(seqpal),limits=M_error,breaks = seq(round(M_error[1],1),round(M_error[2],1),by=0.1), labels = seq(round(M_error[1],1),round(M_error[2],1),by=0.1))+labs(fill='Error')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))

error_legend = get_legend(error_legend)

time_legend = ggplot(time_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=z)) + scale_fill_gradientn(colours=rev(seqpal2),limits=M_time,breaks = squish(c(1000,4000,16000)),labels=c(1000,4000,16000))+labs(fill='Time')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), plot.title=element_text(size=smallfontsize),legend.key =element_blank(),legend.text=element_text(size=smallfontsize))
...
# pdf('/Users/eleanorbrush/Desktop/parameters_interactions.pdf',width=width,height=height,family=fontfamily)

# par(ps=smallfontsize)

# grid.arrange(contour_error[[1]],contour_error[[2]],error_legend,contour_time[[1]],contour_time[[2]],time_legend,ncol=3,widths=c(0.4,0.4,0.12))	
...
grid.arrange(contour_error[[1]],contour_error[[2]],contour_error[[3]],error_legend,contour_time[[1]],contour_time[[2]],contour_time[[3]],time_legend,ncol=4,widths=c(0.4,0.4,0.4,0.16))	

dev.off()

##### optimized cognitive traits 

# # examples = data.frame(N=c(25,25,25,100),rho=c(0.99,0.99,0.99,0.99),w=c(Inf,2500,500,Inf))
# examples = examples[c(3:1,4),]
# examples = data.frame(N=c(25,25,50,100),rho=c(0.99,0.99,0.99,0.99),w=c(500,Inf,Inf,Inf))
# num_ex = dim(examples)[1]

# f_cat = as.vector(find_df(examples,p=parameters_cat))
# f_rule = as.vector(find_df(examples,p=parameters_rule))

# w_vec = seq(0.1,1,by=0.1)
...
# for(j in 1:length(w_vec)){
	...
	# for(i in 1:num_ex){
		# f = find(N=examples$N[i],rho=examples$rho[i],w=examples$w[i],p=parameters_cat[f_cat,])
		# best = f[which.min(optimized_cat[f])]
		# optimal_cognition_cat = rbind(optimal_cognition_cat,data.frame(c=w,N=examples$N[i],rho=examples$rho[i],w=examples$w[i],delta=parameters_cat$delta[f_cat[best]],l=parameters_cat$l[f_cat[best]],cost=optimized_cat[best],time=best_cat[best],error=error_cat[best_cat[best],f_cat[best]]))
		# f = find(N=examples$N[i],rho=examples$rho[i],w=examples$w[i],p=parameters_rule[f_rule,])
		# if(!isempty(f)){
			# optimal_cognition_rule = rbind(optimal_cognition_rule,data.frame(c=w,N=examples$N[i],rho=examples$rho[i],w=examples$w[i],delta=0,l=0.05,cost=optimized_rule[f],time=best_rule[f],error=error_rule[best_rule[f],f_rule[f]]))
		# }
	# }
# }

# optimal_cognition_cat$time = squish(time[optimal_cognition_cat$time])
# optimal_cognition_rule$time = squish(time[optimal_cognition_rule$time])
...
# pdf('/Users/eleanorbrush/Desktop/strategies.pdf',width=width,height=height,family=fontfamily)
...
# for(i in 1:num_ex){
	...
	# if(w_now==Inf){
		# w_now = 30000
	# }
	# rest_of_label = paste('=',examples$rho[i],',N=',examples$N[i],',w=',w_now,sep='')
	# legend_labels = c(legend_labels,paste('N=',examples$N[i],',w=',w_now,sep=''))
	# }
# legend_labels=c(legend_labels,'recognition','rule')
# legend_labels = sapply(legend_labels,as.expression)

# parameters_toplot=c('time','error','l','delta')
# rule_relevant = c('time','error')
# labels = c(expression(paste('Stopping time, ',tau,sep='')),expression(paste('Error, ',epsilon(tau),sep='')),'Updating rate, r',expression(paste('Category width, ',delta,sep='')))

# f_cat = find_df(examples,p=optimal_cognition_cat)
# f_rule = find_df(examples,p=optimal_cognition_rule)

# for(var in parameters_toplot){	
	# if(var=='time' || var =='error'){
		# ltys=rep(1,4)
	# }else{
		# ltys=c(1,1,1,1)
	# }
	
	# ylim = range(c(range(optimal_cognition_cat[[var]]),range(optimal_cognition_rule[[var]])))
	# if(var!='time'){plot(optimal_cognition_cat$c,optimal_cognition_cat[[var]],t='n',xlab='',ylab='',ylim=ylim,xaxt='n',yaxt='n')
		# axis(1,mgp=c(3,0.6,0))
		# axis(2,mgp=c(3,0.6,0))
	# }else{
		# plot(optimal_cognition_cat$c,optimal_cognition_cat[[var]],t='n',xlab='',ylab='',ylim=ylim,xaxt='n',yaxt='n')
		# ticks=c(1,1000,4000,9000,16000,25000)
		# axis(2,at=squish(ticks),labels=ticks,mgp=c(3,0.6,0))
		# axis(1,mgp=c(3,0.6,0))
	# }
	# for(i in 1:num_ex){
		# points(optimal_cognition_cat$c[f_cat[i,]],optimal_cognition_cat[[var]][f_cat[i,]],t='o',col=mycols[i],lty=ltys[i])
	# }	
	# for(i in 1:dim(f_rule)[1]){
		# if(is.element(var,rule_relevant)){
			# # points(optimal_cognition_rule$c[f_rule[i,]],optimal_cognition_rule[[var]][f_rule[i,]],t='o',col=mycols[i],lty=2)
		# }	
	# }	
	# xlim = c(0,1)
	# mtext('Cost of errors, c',1,line=1.75,at=0.5*xlim[1]+0.5*xlim[2],cex=largefontsize/smallfontsize)
	# mtext(labels[which(parameters_toplot==var)],2,line=1.75,at=mean(ylim),cex=largefontsize/smallfontsize)
	# if(var=='time'){
		# legend(0.08,y=10.5,legend=legend_labels[1:num_ex],lty=c(rep(1,num_ex),1,2),col=c(mycols[1:num_ex]),bty='n',seg.len=1.1)
		# # legend(-0.01,y=120,legend=legend_labels,lty=c(rep(1,num_ex),1,2),col=c(mycols[1:num_ex],'black','black'),bty='n',seg.len=1)
	# }
# }
...
examples = data.frame(N=rep(c(25,50,100),each=(xw_cat)*xrho_cat),rho=rep(rep(c(0.5,0.9,0.99),each=xw_cat),times=xN_cat),w=rep(c(250,500,1000,1500,2000,2500,Inf),times=xN_cat*xrho_cat))
N_ex = unique(examples$N)
xN_ex = length(N_ex)
num_ex = dim(examples)[1]/xN_ex
...
for(j in 1:length(w_vec)){
	w = w_vec[j]
	for(i in 1:dim(examples)[1]){		
		f = find(N=examples$N[i],rho=examples$rho[i],p=parameters_genetic)
		optimal_cognition_gen = rbind(optimal_cognition_gen,data.frame(c=w,N=examples$N[i],rho=examples$rho[i],w=examples$w[i],cost=w*2*error_genetic[1,f],time=1,error=error_genetic[1,f]))
	}
}
...
for(i in 1:3){
	for(j in 1:3){
		...
		f_cat_now = find(N=unique(examples$N)[i],rho=unique(examples$rho)[j],p=optimal_cognition_cat)
		f_rule_now = find(N=unique(examples$N)[i],rho=unique(examples$rho)[j],p=optimal_cognition_rule)
		f_gen_now = find(N=unique(examples$N)[i],rho=unique(examples$rho)[j],p=optimal_cognition_gen)
		...
		contour_diff[[I]] = ggplot(diff_df,aes(x=c,y=w,z=diff)) +
			...
			scale_fill_gradientn(colours=rev(divpal),limits=M_diff,guide='colorbar')+			
			...
			scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) + scale_x_continuous(expand = c(0,0),breaks=seq(0,1,by=0.25),limits=c(0.45,1.05))+
...
diff_legend = ggplot(diff_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=z)) + scale_fill_gradientn(colours=rev(divpal),limits=M_diff,breaks = seq(round(M_diff[1],1),round(M_diff[2],1),by=0.1), labels = seq(round(M_diff[1],1),round(M_diff[2],1),by=0.1))+labs(fill='Difference')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))
...
grid.arrange(contour_diff[[4]],contour_diff[[5]],contour_diff[[6]],diff_legend,contour_diff[[7]],contour_diff[[8]],contour_diff[[9]],ncol=4,widths=c(0.4,0.4,0.4,0.15))

dev.off()

####best assessment strategy

rainpal = c(brewer.pal(5,'RdBu')[1],brewer.pal(11,'RdBu')[c(8:10)],brewer.pal(11,'Spectral')[11])
contour_diff = list()
hold = list()
...
for(i in 1:3){
	for(j in 1:3){
		...
		contour_diff[[I]] = ggplot(diff_df,aes(x=c,y=w,z=diff)) +
			geom_tile(aes(fill=diff))+
			scale_fill_gradientn(colours=(rainpal),limits=M_diff,guide='colorbar')+		
...
diff_legend = ggplot(diff_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=z)) + scale_fill_manual(values=(rainpal),labels =c('Categ','Learned','Innate','Either','Any'))+labs(fill='')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))
...
grid.arrange(contour_diff[[4]],contour_diff[[5]],contour_diff[[6]],diff_legend,contour_diff[[7]],contour_diff[[8]],contour_diff[[9]],ncol=4,widths=c(0.4,0.4,0.4,0.17))
...
seqpal = rev(brewer.pal(9,'Reds'))
seqpal2 = rev(brewer.pal(9,'Blues')[2:9])
# discpal = seqpal[c(6,2)]
discpal = brewer.pal(9,'Greens')[c(2,6)]
discpal2 = brewer.pal(9,'Purples')[c(2,4,6,8)]
...
for(i in 1:3){
	for(j in 1:3){
		...
		contour_delta[[I]] = ggplot(delta_df,aes(x=c,y=w,z=delta)) +
			...
			scale_fill_gradientn(colours=discpal,limits=M_delta,guide='colorbar')+
			theme_bw()+ theme(text=element_text(family="Helvetica", size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
			scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) + scale_x_continuous(expand = c(0,0),breaks=seq(0,1,by=0.25),limits=c(0.45,1.05))+
			ylab('Memory window, w') + xlab('Cost of errors, c')
	}
...
# delta_legend = ggplot(delta_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=z)) + scale_fill_gradientn(colours=rev(seqpal),limits=M_delta,breaks = seq(0,M_delta[2],by=0.25), labels = seq(0,M_delta[2],by=0.25))+labs(fill='')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))

delta_legend = ggplot(delta_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=factor(z))) + scale_fill_manual(values=rev(discpal),labels=c(0.25,0))+labs(fill='Cat width')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))
...
for(i in 1:3){
	for(j in 1:3){
		...
		contour_l[[I]] = ggplot(l_df,aes(x=c,y=w,z=l)) +
			...
			scale_fill_manual(values=discpal2,limits=levels(l_df$l))+
			theme_bw()+ theme(text=element_text(family="Helvetica", size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position='none')+
			scale_y_continuous(expand = c(0,0),breaks=seq(000,3000,by=500),labels=c(250,seq(500,2500,by=500),30000)) + scale_x_continuous(expand = c(0,0),breaks=seq(0,1,by=0.25),limits=c(0.45,1.05))+
			ylab('Memory window, w') + xlab('Cost of errors, c')
	}
...
l_legend = ggplot(l_fake,aes(x=x,y=y,z=z)) + geom_tile(aes(fill=factor(z))) + scale_fill_manual(values=rev(discpal2),labels=rev(unique(parameters_cat$l)))+labs(fill='Rate')+theme(text=element_text(family="Helvetica", size=smallfontsize), plot.title=element_text(size=smallfontsize), plot.margin=unit(marg,"cm"), legend.key =element_blank(),legend.text=element_text(size=smallfontsize))
...
grid.arrange(contour_l[[1]],contour_l[[2]],contour_l[[3]],l_legend,contour_l[[4]],contour_l[[5]],contour_l[[6]],blank,contour_l[[7]],contour_l[[8]],contour_l[[9]],ncol=4,widths=c(0.4,0.4,0.4,0.15))
...
grid.arrange(contour_time[[1]],contour_time[[2]],contour_time[[3]],time_legend,contour_time[[4]],contour_time[[5]],contour_time[[6]],blank,contour_time[[7]],contour_time[[8]],contour_time[[9]],ncol=4,widths=c(0.4,0.4,0.4,0.15))
...
grid.arrange(contour_error[[1]],contour_error[[2]],contour_error[[3]],error_legend,contour_error[[4]],contour_error[[5]],contour_error[[6]],blank,contour_error[[7]],contour_error[[8]],contour_error[[9]],ncol=4,widths=c(0.4,0.4,0.4,0.15))
...
grid.arrange(contour_time[[7]],contour_time[[8]],contour_time[[9]],time_legend,contour_l[[7]],contour_l[[8]],contour_l[[9]],l_legend,contour_delta[[7]],contour_delta[[8]],contour_delta[[9]],delta_legend,contour_error[[7]],contour_error[[8]],contour_error[[9]],error_legend,ncol=4,widths=c(0.4,0.4,0.4,0.15))
