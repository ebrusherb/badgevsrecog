...
n = toplot$n[q]
	...
	if(length(which(error$fights>Tfights_min))!=0){
			error=error[-which(error$fights>Tfights_min),]}
	error$categ=revalue(error$categ,c('Categ'='Badge','Indiv'='Indiv'))
...
# ##---- error as a function of time early
Tfights_min = 500
plots=list()
...
n = toplot$n[q]
	...
	plots[[q]] = ggplot(error, aes(x=fights, y =error, colour = categ, linetype = sigcorr)) + 
		geom_line() + 
		scale_y_continuous(limits=c(0,0.5)) + scale_x_continuous(breaks=seq(0,Tfights_min,by=100)) + 
		theme_bw() +
		theme(text=element_text(family="Helvetica", size=10),plot.title=element_text(size=10) , plot.margin=unit(c(0,0.25,0,0),"cm")) + 
		scale_linetype_manual(values=c(3,1)) +
		scale_color_manual(values=divpal[c(1,5)])+
...
pdf(file=paste(wd,"/learning_curves_early.pdf",sep=""),width=6.83,height=3)				
grid.arrange(plots[[1]],plots[[2]],plots[[3]],legend,plots[[4]],plots[[5]],plots[[6]],ncol=4,widths=c(1,1,1,.5))
dev.off()
...
plots = list()	
	...
	windandtype[windandtype==0] = paste('Badge, ','Wind = ',wind_vals[wvals[1]],sep='')
	windandtype[windandtype==1] = paste('Badge, ','Wind = ',wind_vals[wvals[2]],sep='')
	...
	stacked = with(error,data.frame(error=c(categ,indiv),categ=factor(rep(c('Badge','indiv'),each=prod(c(xN,xw,xcorr2)))),groupsize=rep(groupsize,2),windandtype = as.factor(windandtype),sigcorr = rep(sigcorr,2)))
	...
	error$window[error$window==wind_vals[wvals[1]]] = paste('Badge, ','Wind = ',wind_vals[wvals[1]],sep='')
	error$window[error$window==wind_vals[wvals[2]]] = paste('Badge, ','Wind = ',wind_vals[wvals[2]],sep='')
...
pdf(file=paste(wd,"/summary_error.pdf",sep=""),width=6.83,height=3)		
...
plots=list()
	...
	windandtype[windandtype==2] = paste('Indiv, ','Wind = ',wind_vals[wvals[1]],sep='')
	windandtype[windandtype==3] = paste('Indiv, ','Wind = ',wind_vals[wvals[2]],sep='')
	stacked = with(time,data.frame(time=c(categ,indiv),categ=factor(rep(c('Badge','indiv'),each=prod(c(xN,xw,xcorr2)))),groupsize=rep(groupsize,2),windandtype = as.factor(windandtype),sigcorr = rep(sigcorr,2)))
	stacked=stacked[-intersect(which(stacked$categ=='indiv'),which(stacked$sigcorr==corr_vals[c2vals[1]])),]
	...
	time$window[error$window==wind_vals[wvals[1]]] = paste('Badge, ','Wind = ',wind_vals[wvals[1]],sep='')
	time$window[error$window==wind_vals[wvals[2]]] = paste('Badge, ','Wind = ',wind_vals[wvals[2]],sep='')
...
pdf(file=paste(wd,"/summary_time.pdf",sep=""),width=6.83,height=3)		
grid.arrange(plots[[1]],plots[[2]],legend,ncol=3,widths=c(1,1,0.5))
dev.off()
